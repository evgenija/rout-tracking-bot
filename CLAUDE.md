...
### Timestamps: P1 vs P2 — різні timezone
P1 `routes.start_time`/`end_time` — з offset (`+03:00`, Kyiv).
P2 `daily_input.created_at` — naive UTC (без offset). Порівнюючи час між P1 і P2 — конвертувати вручну (+3 год), інакше здається що `daily_input` створився РАНІШЕ фінішу маршруту (насправді співпадає).
...
## driver_type — архітектура (з 12.09.2026)

Джерело істини для тарифу: `route_cost_service.py` → `OWN_DRIVER_IDS` (denylist).
Будь-який telegram_id НЕ в цьому списку → автоматично `'logistics'`.

Поточний `OWN_DRIVER_IDS`: Трохимчук (5865598299), Тарас (568294118),
Юрій (496946423, неактивний — залишений `'own'` за рішенням Жені).

`create_user()` в `database.py` пише `driver_type='logistics'` за замовчуванням
для нових водіїв — консистентно з `users.driver_type` (яку читає `km_reader.py`).

⚠️ До 12.09.2026 було 2 неузгоджені джерела істини: хардкод-сет `LOGISTICS_DRIVER_IDS`
(allowlist, впливав на вартість) і колонка `users.driver_type` (впливала тільки на звіти
`km_reader.py`). Уніфіковано через перехід на denylist + дефолт у `create_user()`.
...
**Верифікація 15.07.2026:** маршрут #324 (29 waypoints) перевірено після фіксу chunks (5c78b25/bcfd0c0) — усі точки враховані, жодна не загублена, `route_polyline` цілий (не NULL). Fix підтверджено робочим на реальних даних.

---

## Діагностика великої похибки (одометр vs GPS-трекінг)

Якщо похибка на маршруті висока (>15–20%) — це не завжди баг chunks/sampling. Перевірити:

1. Waypoints фіксуються тільки на зупинках (без авто-GPS між ними — свідоме рішення заради безпеки водіїв) → Google Directions рахує «пряму» дорогу між двома пінгами і фізично не бачить реальний детур між ними.
2. **Ознака легітимної (не-багової) похибки:** відрізок з великим `gap_min` і малою `gap_km`/швидкістю (напр. 80 хв їзди на 13 км прямої = ~10 км/год) — це реальна поїздка (заправка/база/затори), яку трекінг фізично не міг вловити.
3. **Ознака БАГА** (sampling/chunks): точка з `is_suspicious=True`, яка виключає легітимний відрізок, або відсутній `route_polyline` при `is_active=0`.
4. Скрипт-шаблон для діагностики: `scripts/_tmp_check_route_324.py` — виводить waypoints з `gap_km`/`gap_min`/`speed_kmh` по кожному відрізку.

**Приклад 15.07.2026 (#324):** похибка 26% (одометр 272.0 vs трекінг 215.6 км) — легітимна, не баг. Відрізок «матяш → Фініш»: 80.9 хв на 13.6 км прямої = ознака #2. Формула ×1.099 застосувалась коректно (`daily_input.km = 236.97`).
